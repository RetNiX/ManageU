...
function showActions() {
    ...
    for (let action of manager.actions) {
        document.getElementById("tableData").innerHTML += `<tr class=${action.type == "income" ? "table-success" : "table-danger"}><td>${action.description}</td><td>${action.amount}</td><td><i class="fa-solid fa-pen-to-square" onclick="updateAction(${action.id})" style="color: #5188e6;"></i></td><td><i class="fa-solid fa-trash-can" onclick="deleteAction(${action.id})" style="color: #5188e6;"></i></td></tr>`;
    }
}
...
function showNewAction() {
    document.getElementById("tableData").innerHTML += `<tr class=${manager.actions[manager.actions.length - 1].type == "income" ? "table-success" : "table-danger"}><td>${manager.actions[manager.actions.length - 1].description}</td><td>${manager.actions[manager.actions.length-1].amount}</td><td><i class="fa-solid fa-pen-to-square" onclick="updateAction(${manager.actions[manager.actions.length-1].id})" style="color: #5188e6;"></i></td><td><i class="fa-solid fa-trash-can" onclick="deleteAction(${manager.actions[manager.actions.length-1].id})" style="color: #5188e6;"></i></td></tr>`;
}
